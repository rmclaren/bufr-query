...
class ObsBuilder:
    ...
    def make_obs(self, comm) -> bufr.DataContainer:
        return bufr.DataContainer(self.input_path, self.make_description(), comm)

    def create_obs_group(self, category, env):
        if not ioda_available:
            raise ImportError('puioda was not found. Cant make obsgroup objects.')
