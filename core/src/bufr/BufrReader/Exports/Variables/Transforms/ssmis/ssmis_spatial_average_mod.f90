...
!
! Program history log:
!    2011-11-18   collard   - Original version (for SSMIS)
...
!
  ...
  use ssmis_kinds, only: r_kind,r_double,i_kind
  use m_distance
  use m_uniq
...
!      Put data into 2D (fov vs. scanline) array
       do iobs = 1, num_obs
          ...
          bt_image_orig(fov(iobs),scanline(iobs),:)= bt_obs(:,iobs)
...
!      Put data in 2D (fov,scanline) grid to perform noise reduction
       do i=1,num_obs
          ...
          bt_image_orig(fov(i),scanline(i),:)= bt_obs(:,i) 
       ...
       do iscan = 1, max_scan
          do ifov = 1, max_fov
             if (scanline_back(ifov,iscan) >0) then 
                bt_obs(:,scanline_back(ifov,iscan)) = bt_image(ifov,iscan,:)
...
!      write(*,*) 'min/max scanline      = ', minval(scanline), maxval(scanline) 
!      write(*,*) 'min/max scan          = ', minval(scan), maxval(scan) 
!      write(*,*) 'min/max bt_obs        ', minval(bt_obs), maxval(bt_obs) 
...
!      Noise reduction using FFT
       ...
       DO IChan=1,nchanl
          ...
          ! (otherwise bt_obs just keeps the same value):
                ...
                do iscan=1,max_scan
                   do ifov=1,max_fov
                      if (Scanline_Back(IFov, IScan) > 0) &
                           bt_obs(channelnumber(ichan),Scanline_Back(IFov, IScan)) = &
